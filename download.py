...
def download_core_dataset(s3_base_url, dataset_dir):
    ...
    for subpath in core_subpaths:
        ...
        if not fullpath.exists():
            # Properly format the S3 URL
            s3_url = f"{s3_base_url.rstrip('/')}/{subpath.as_posix().lstrip('/')}"
            print(f"Downloading {s3_url} to {fullpath}")

            # Construct the AWS CLI command
            cmd = [
                sys.executable, "-m", "awscli", "s3", "cp",
                "--no-sign-request", s3_url, str(fullpath)
            ]
            ...
            try:
                ...
                print(f"Error downloading {s3_url}: {e}", file=sys.stderr)
                continue
